...
console.log('[City] BUILD v25 DOOR FIX + START BTN');
...
const CONFIG = {
  ...
  worldSize: 80, camModes: ['1st', '3rd', 'Front']
};
...
function init() {
  // 1) Attach start button FIRST so a later crash never blocks it
  ...
  if (startBtn) {
    ...
    console.log('[City] start-btn listeners attached early');
  }

  try {
    scene = new THREE.Scene();
    scene.background = new THREE.Color(0x87B8D8);
    scene.fog = new THREE.Fog(0x87B8D8, 40, 120);
    ...
    const road = new THREE.Mesh(
      new THREE.PlaneGeometry(12, CONFIG.worldSize),
    ...
    // World generation wrapped so a model error never kills the start button
    try {
      buildTestHouse();
      console.log('[City] buildTestHouse ok');
    } catch (e) {
      console.error('[City] buildTestHouse FAILED', e);
    }
    try {
      buildInteriorRoom();
      console.log('[City] buildInteriorRoom ok');
    } catch (e) {
      console.error('[City] buildInteriorRoom FAILED', e);
    }
    try {
      buildPlayerBody();
      console.log('[City] buildPlayerBody ok');
    } catch (e) {
      console.error('[City] buildPlayerBody FAILED', e);
    }

    // Assign DOM refs BEFORE setupControls
    ...
    animate();
    console.log('[City] v25 ready — doors', doors.length, 'colliders', colliders.length);
  } catch (e) {
    console.error('[City] init FATAL', e);
    // Still try to keep the start button alive even if renderer dies
  }
}
...
function buildTestHouse() {
  ...
  door.position.set(dw * 0.5, dh / 2, 0);
  hinge.add(door);
  const handle = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.1, 0.14), new THREE.MeshLambertMaterial({ color: 0xd4a84b }));
  handle.position.set(dw * 0.85, dh * 0.5, 0.08);
  hinge.add(handle);
  g.add(hinge);
  g.position.set(cx, py, cz);
  scene.add(g);
  colliders.push({
    min: new THREE.Vector3(cx - bw / 2, py, cz - bd / 2),
    max: new THREE.Vector3(cx + bw / 2, py + bh + 1, cz + bd / 2)
  });
  const frontZ = cz + bd / 2;
  doors.push({
    type: 'house', hinge, openAngle: -Math.PI * 0.62,
    dx: cx, dy: py + 1.1, dz: frontZ,
    exitSpot: new THREE.Vector3(cx, py + CONFIG.playerHeight, frontZ + 1.8)
  });
  ...
  for (const [x, z, col, label] of [[14, -2, 0xc0392b, 'eatery'], [-14, -2, 0x3498db, 'hospital'], [0, -16, 0xf9e79f, 'school']]) {
    ...
    hh.position.set(-0.6, 0, 3.1);
    const dd = new THREE.Mesh(new THREE.BoxGeometry(1.2, 2.1, 0.1), new THREE.MeshLambertMaterial({ color: 0x2a1810 }));
    dd.position.set(0.6, 1.05, 0);
    hh.add(dd);
    gg.add(hh);
    gg.position.set(x, 0, z);
    scene.add(gg);
    colliders.push({
      min: new THREE.Vector3(x - 3.5, 0, z - 3),
      max: new THREE.Vector3(x + 3.5, 9, z + 3)
    });
    doors.push({
      type: label, hinge: hh, openAngle: -Math.PI * 0.62,
      dx: x, dy: 1.1, dz: z + 3,
      exitSpot: new THREE.Vector3(x, CONFIG.playerHeight, z + 3 + 1.8)
    });
  }
}
...
function buildInteriorRoom() {
  ...
  floor.position.set(INT_ORIGIN.x, INT_ORIGIN.y, INT_ORIGIN.z);
  ...
  walls.position.set(INT_ORIGIN.x, INT_ORIGIN.y + 2, INT_ORIGIN.z);
  interiorGroup.add(walls);
  const light = new THREE.PointLight(0xfff0d0, 1.2, 20);
  light.position.set(INT_ORIGIN.x, INT_ORIGIN.y + 3.5, INT_ORIGIN.z);
  ...
  table.position.set(INT_ORIGIN.x, INT_ORIGIN.y + 0.75, INT_ORIGIN.z);
  ...
  couch.position.set(INT_ORIGIN.x, INT_ORIGIN.y + 0.4, INT_ORIGIN.z - 2.5);
...
function makeCharacter(shirtCol, opts = {}) {
  ...
  const thighL = new THREE.Mesh(new THREE.CylinderGeometry(0.075, 0.068, 0.34, 8), pants);
  thighL.position.y = -0.17;
  legL.add(thighL);
  const shinL = new THREE.Mesh(new THREE.CylinderGeometry(0.062, 0.052, 0.3, 8), pants);
  shinL.position.y = -0.47;
  legL.add(shinL);
  const shoeL = new THREE.Mesh(new THREE.BoxGeometry(0.13, 0.08, 0.22), shoe);
  shoeL.position.set(0, -0.64, 0.04);
  legL.add(shoeL);
  g.add(legL);

  const legR = new THREE.Group();
  legR.position.set(0.12, 0.78, 0);
  const thighR = new THREE.Mesh(new THREE.CylinderGeometry(0.075, 0.068, 0.34, 8), pants);
  thighR.position.y = -0.17;
  legR.add(thighR);
  const shinR = new THREE.Mesh(new THREE.CylinderGeometry(0.062, 0.052, 0.3, 8), pants);
  shinR.position.y = -0.47;
  legR.add(shinR);
  const shoeR = new THREE.Mesh(new THREE.BoxGeometry(0.13, 0.08, 0.22), shoe);
  shoeR.position.set(0, -0.64, 0.04);
  legR.add(shoeR);
  g.add(legR);

  const hips = new THREE.Mesh(new THREE.CylinderGeometry(0.17, 0.155, 0.15, 10), pants);
  hips.position.y = 0.82;
  g.add(hips);

  const torso = new THREE.Mesh(new THREE.CylinderGeometry(0.155, 0.175, 0.44, 10), shirt);
  torso.position.y = 1.1;
  g.add(torso);

  const jacketMesh = new THREE.Mesh(new THREE.CylinderGeometry(0.175, 0.19, 0.4, 10), jacket);
  jacketMesh.position.y = 1.12;
  g.add(jacketMesh);

  const armL = new THREE.Group();
  armL.position.set(-0.23, 1.26, 0);
  const upperL = new THREE.Mesh(new THREE.CylinderGeometry(0.048, 0.042, 0.3, 7), jacket);
  upperL.position.y = -0.15;
  armL.add(upperL);
  const lowerL = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.036, 0.26, 7), skin);
  lowerL.position.y = -0.41;
  armL.add(lowerL);
  const handL = new THREE.Mesh(new THREE.SphereGeometry(0.042, 6, 5), skin);
  handL.position.y = -0.56;
  armL.add(handL);
  g.add(armL);

  const armR = new THREE.Group();
  armR.position.set(0.23, 1.26, 0);
  const upperR = new THREE.Mesh(new THREE.CylinderGeometry(0.048, 0.042, 0.3, 7), jacket);
  upperR.position.y = -0.15;
  armR.add(upperR);
  const lowerR = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.036, 0.26, 7), skin);
  lowerR.position.y = -0.41;
  armR.add(lowerR);
  const handR = new THREE.Mesh(new THREE.SphereGeometry(0.042, 6, 5), skin);
  handR.position.y = -0.56;
  armR.add(handR);
  g.add(armR);

  const neck = new THREE.Mesh(new THREE.CylinderGeometry(0.052, 0.06, 0.11, 8), skin);
  neck.position.y = 1.36;
  g.add(neck);
  const head = new THREE.Mesh(new THREE.SphereGeometry(0.14, 12, 10), skin);
  head.position.y = 1.52;
  g.add(head);
  const hair = new THREE.Mesh(new THREE.SphereGeometry(0.145, 10, 8), hairM);
  hair.position.y = 1.58;
  hair.scale.set(1.05, 0.72, 1.05);
  g.add(hair);
  const fringe = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.05, 0.06), hairM);
  fringe.position.set(0, 1.58, 0.11);
  g.add(fringe);

  const eyeW = new THREE.MeshLambertMaterial({ color: 0xffffff });
  const eyeP = new THREE.MeshLambertMaterial({ color: 0x1a1a2e });
  const eL = new THREE.Mesh(new THREE.SphereGeometry(0.03, 6, 5), eyeW);
  eL.position.set(-0.048, 1.54, 0.115);
  g.add(eL);
  const pL = new THREE.Mesh(new THREE.SphereGeometry(0.015, 5, 4), eyeP);
  pL.position.set(-0.048, 1.54, 0.135);
  g.add(pL);
  const eR = new THREE.Mesh(new THREE.SphereGeometry(0.03, 6, 5), eyeW);
  eR.position.set(0.048, 1.54, 0.115);
  g.add(eR);
  const pR = new THREE.Mesh(new THREE.SphereGeometry(0.015, 5, 4), eyeP);
  pR.position.set(0.048, 1.54, 0.135);
  g.add(pR);

  const browL = new THREE.Mesh(new THREE.BoxGeometry(0.055, 0.014, 0.02), hairM);
  browL.position.set(-0.048, 1.58, 0.125);
  g.add(browL);
  const browR = new THREE.Mesh(new THREE.BoxGeometry(0.055, 0.014, 0.02), hairM);
  browR.position.set(0.048, 1.58, 0.125);
  g.add(browR);
  const nose = new THREE.Mesh(new THREE.SphereGeometry(0.022, 5, 4), skin);
  nose.position.set(0, 1.51, 0.135);
  g.add(nose);
  const mouth = new THREE.Mesh(new THREE.BoxGeometry(0.055, 0.014, 0.016), new THREE.MeshLambertMaterial({ color: 0xb06060 }));
  mouth.position.set(0, 1.455, 0.13);
  g.add(mouth);
...
function updateInteract() {
  ...
  if (debugEl) {
    ...
    debugEl.innerHTML =
      'DOOR DBG v25<br>' +
      'doors:' + doors.length + ' col:' + colliders.length + '<br>' +
      'dist:' + (nearestDist < 900 ? nearestDist.toFixed(2) : '—') + ' dot:' + (nearestDot > -1 ? nearestDot.toFixed(2) : '—') + '<br>' +
      'btnHidden:' + hid + ' target:' + (currentDoorTarget ? (currentDoorTarget === 'exit' ? 'exit' : currentDoorTarget.type) : 'none') + '<br>' +
      'anim:' + (doorAnim ? doorAnim.mode : '—') + '<br>' +
...
function updateDoorAnim(dt) {
  ...
  if (k >= 1 && !doorAnim.done) {
    ...
    doorAnim = null;
    if (mode === 'open') {
      console.log('[City]enterInterior', door.type);
      enterInterior(door);
    }
  }
}
...
function setupControls() {
  ...
  if (lookZone) {
    lookZone.addEventListener('touchstart', e => {
      ...
      lastLookX = t0.clientX;
      lastLookY = t0.clientY;
    }, { passive: false });
...
function updatePlayer(dt) {
  ...
  _w.set(0, 0, 0);
  _w.addScaledVector(_f, my);
  _w.addScaledVector(_r, mx);
  ...
  if (!interiorActive) {
    nx = THREE.MathUtils.clamp(nx, -35, 35);
    nz = THREE.MathUtils.clamp(nz, -35, 35);
  } else {
...
function updateCamera() {
  if (camMode === 0) {
    ...
    camera.lookAt(
      player.pos.x - Math.sin(player.yaw) * cp * 2,
      player.pos.y + 0.15 + sp * 2,
      player.pos.z - Math.cos(player.yaw) * cp * 2
    );
  } else if (camMode === 1) {
    const back = 3.5, up = 1.8;
    camera.position.set(
      player.pos.x + Math.sin(player.yaw) * back,
      player.pos.y + up,
      player.pos.z + Math.cos(player.yaw) * back
    );
