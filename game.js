...
function init(){
  ...
  try{
    buildGround(); placeRoads(); placeBuildings(); placeProps();
    spawnCars(); spawnNpcs(); spawnAnimals();
    buildPlayerBody(); buildInteriorRoom();
  }catch(e){console.error('[City] build error',e);}
  console.log('[City] BUILD v19 children:',scene.children.length,'doors:',doors.length,'lights:',trafficLights.length);
...
function playFootstep(){
  ...
  const g=audioCtx.createGain(); g.gain.setValueAtTime(0.08,t); g.gain.exponentialRampToValueAtTime(0.001,t+0.05);
...
function placeRoads(){
  const cell=CONFIG.blockSize+CONFIG.streetWidth, half=CONFIG.worldSize*0.42, sw=CONFIG.streetWidth;
  const lineM=new THREE.MeshBasicMaterial({color:0xf5f5e8});
  const zebraM=new THREE.MeshBasicMaterial({color:0xffffff});
  const stopM=new THREE.MeshBasicMaterial({color:0xffee88});
  for(let g=-half;g<=half;g+=cell){
    const zc=g+sw*0.5, xc=g+sw*0.5;
    for(let x=-half;x<half;x+=4.2){
      const dash=new THREE.Mesh(new THREE.BoxGeometry(2.0,0.05,0.12),lineM);
      dash.position.set(x+1.0,0.04,zc); scene.add(dash);
    }
    for(let z=-half;z<half;z+=4.2){
      const dash=new THREE.Mesh(new THREE.BoxGeometry(0.12,0.05,2.0),lineM);
      dash.position.set(xc,0.04,z+1.0); scene.add(dash);
    }
  }
  let tl=0;
  for(let gx=-half;gx<=half;gx+=cell){
    for(let gz=-half;gz<=half;gz+=cell){
      ...
      for(let s=-3;s<=3;s++){
        const a=new THREE.Mesh(new THREE.BoxGeometry(0.45,0.055,sw*0.85),zebraM);
        a.position.set(ix+s*0.7,0.045,iz); scene.add(a);
        const b=new THREE.Mesh(new THREE.BoxGeometry(sw*0.85,0.055,0.45),zebraM);
        b.position.set(ix,0.045,iz+s*0.7); scene.add(b);
      }
      if(tl<16){
  ...
  console.log('[City] traffic lights',trafficLights.length);
}
function addTrafficLight(x,z,rotY){
  const g=new THREE.Group();
  const pole=new THREE.Mesh(new THREE.CylinderGeometry(0.08,0.1,3.6,6),new THREE.MeshLambertMaterial({color:0x333340}));
  pole.position.y=1.8; g.add(pole);
  const head=new THREE.Mesh(new THREE.BoxGeometry(0.35,0.95,0.28),new THREE.MeshLambertMaterial({color:0x1a1a1a}));
  head.position.y=3.7; g.add(head);
  const red=new THREE.Mesh(new THREE.SphereGeometry(0.1,8,6),new THREE.MeshBasicMaterial({color:0x330000}));
  red.position.set(0,4.0,0.12); g.add(red);
  const yel=new THREE.Mesh(new THREE.SphereGeometry(0.1,8,6),new THREE.MeshBasicMaterial({color:0x332200}));
  yel.position.set(0,3.7,0.12); g.add(yel);
  const grn=new THREE.Mesh(new THREE.SphereGeometry(0.1,8,6),new THREE.MeshBasicMaterial({color:0x003300}));
  grn.position.set(0,3.4,0.12); g.add(grn);
  g.position.set(x,getHeight(x,z),z); g.rotation.y=rotY; scene.add(g);
  const phaseOff=(Math.abs(x)+Math.abs(z))*0.07;
  trafficLights.push({red,yel,grn,timer:phaseOff%12,state:0,x,z});
  setTL(trafficLights[trafficLights.length-1]);
}
function setTL(tl){
  tl.red.material.color.setHex(tl.state===2?0xff2222:0x330000);
  tl.yel.material.color.setHex(tl.state===1?0xffcc00:0x332200);
  tl.grn.material.color.setHex(tl.state===0?0x22ff44:0x003300);
}
...
function lightStateNear(x,z,axis,dir){
  for(const tl of trafficLights){
    if(axis==='x'){
      ...
      const ahead=(tl.x-x)*dir;
      if(ahead>0&&ahead<9&&tl.state!==0) return true;
    } else {
      if(Math.abs(tl.x-x)>5) continue;
      const ahead=(tl.z-z)*dir;
      if(ahead>0&&ahead<9&&tl.state!==0) return true;
    }
  }
  return false;
...
function makeBldg(typeKey,bw,bh,bd){
  ...
  if(bh>4.5){
    const maxRowY=bh-1.0, rowH=2.1;
    const rows=Math.max(1,Math.min(4,Math.floor((maxRowY-1.6)/rowH)+1));
    const cols=Math.max(2,Math.floor(bw/2.1));
    for(let r=0;r<rows;r++) for(let c=0;c<cols;c++){
      if(Math.random()<0.15) continue;
      const wy=1.6+r*rowH;
      if(wy+0.35>bh-0.3) continue;
      const w=new THREE.Mesh(new THREE.BoxGeometry(0.45,0.55,0.06),Math.random()<0.2?winLit:winMat);
      w.position.set(-bw/2+1+c*((bw-2)/Math.max(1,cols-1)),wy,bd/2+0.04);
      g.add(w);
    }
  }
  ...
  for(let gx=-half;gx<half;gx+=cell){
    for(let gz=-half;gz<half;gz+=cell){
      ...
      if(t.enter){
        ...
        doors.push({type:typeKey,min:new THREE.Vector3(cx-0.85,py,cz+bd/2-0.3),max:new THREE.Vector3(cx+0.85,py+2.4,cz+bd/2+1.0),exitSpot:new THREE.Vector3(cx,py+CONFIG.playerHeight,cz+bd/2+1.5)});
        enterCount++;
      } else {
...
function updateInteract(){
  ...
  if(!interiorActive){
    ...
    interactEl.textContent='OPEN DOOR';
...
function makeCar(color,taxi){
  ...
  const body=new THREE.Mesh(new THREE.BoxGeometry(1.9,0.4,0.9),bodyMat); body.position.y=0.35; g.add(body);
  const cabin=new THREE.Mesh(new THREE.BoxGeometry(0.9,0.35,0.82),glass); cabin.position.set(-0.1,0.7,0); g.add(cabin);
  const hood=new THREE.Mesh(new THREE.BoxGeometry(0.5,0.12,0.85),bodyMat); hood.position.set(0.65,0.52,0); g.add(hood);
  for(const wz of [0.3,-0.3]){
    const hl=new THREE.Mesh(new THREE.BoxGeometry(0.08,0.1,0.14),new THREE.MeshBasicMaterial({color:0xfff8e0})); hl.position.set(0.98,0.38,wz); g.add(hl);
    const tl=new THREE.Mesh(new THREE.BoxGeometry(0.06,0.08,0.12),new THREE.MeshBasicMaterial({color:0xff2222})); tl.position.set(-0.98,0.38,wz); g.add(tl);
  }
  if(taxi){const sign=new THREE.Mesh(new THREE.BoxGeometry(0.35,0.12,0.2),new THREE.MeshBasicMaterial({color:0xffdd00})); sign.position.set(-0.1,0.95,0); g.add(sign);}
  const wheels=[];
  for(const [wx,wz] of [[0.5,0.42],[0.5,-0.42],[-0.5,0.42],[-0.5,-0.42]]){
    const w=new THREE.Mesh(new THREE.CylinderGeometry(0.15,0.15,0.1,8),wheelMat); w.rotation.z=Math.PI/2; w.position.set(wx,0.15,wz); g.add(w); wheels.push(w);
  }
...
function spawnCars(){
  ...
  for(let gz=-half;gz<half;gz+=cell){
    ...
    body.position.set((Math.random()-0.5)*CONFIG.worldSize*0.6,0.5,gz+CONFIG.streetWidth*0.5+(dir>0?-1.1:1.1));
    body.rotation.y=dir>0?Math.PI/2:-Math.PI/2; scene.add(body);
    cars.push({mesh:body,axis:'x',dir,speed:7+Math.random()*4,baseSpeed:7+Math.random()*4,bound:half}); id++;
  }
  for(let gx=-half;gx<half;gx+=cell){
    if(Math.abs(gx)<cell*0.5||id>=16) continue;
    const taxi=id%4===0;
    const body=makeCar(taxi?0xf1c40f:colors[id%colors.length],taxi);
    const dir=id%2===0?1:-1;
    body.position.set(gx+CONFIG.streetWidth*0.5+(dir>0?-1.1:1.1),0.5,(Math.random()-0.5)*CONFIG.worldSize*0.6);
    body.rotation.y=dir>0?0:Math.PI; scene.add(body);
    cars.push({mesh:body,axis:'z',dir,speed:7+Math.random()*4,baseSpeed:7+Math.random()*4,bound:half}); id++;
  }
}
function updateCars(dt){
  ...
  for(const c of cars){
    ...
    if(lightStateNear(px,pz,c.axis,c.dir)) c.speed=Math.max(0,c.speed-18*dt);
    else c.speed+=(c.baseSpeed-c.speed)*Math.min(1,dt*2);
    ...
    if(c.mesh.userData.wheels){const spin=c.speed*dt*2.2;for(const w of c.mesh.userData.wheels)w.rotation.x+=spin;}
  }
}
