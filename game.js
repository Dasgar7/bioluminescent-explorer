...
const CONFIG={
 ...
 maxPitch:Math.PI/2.25,worldSize:280,terrainRes:48,blockSize:14,streetWidth:9,
...
const INTERACT_DIST=4.5,INTERACT_DOT=0.84;
let trafficLights=[];
...
const BTYPES={
 ...
 hospital:{cols:[0xecf0f1,0xffffff],roof:0x3498db,h:[12,18],label:'HOSPITAL',sign:0xe74c3c,enter:true},
 school:{cols:[0xf9e79f,0xf5d76e],roof:0x1a5276,h:[9,14],label:'SCHOOL',sign:0x2980b9,enter:true},
 police:{cols:[0x2c3e50,0x34495e],roof:0x1a1a2e,h:[8,12],label:'POLICE',sign:0x3498db,enter:true},
 office:{cols:GLASS,roof:0x1a1a2e,h:[16,28],label:null,sign:null,enter:false},
 shop:{cols:BRICK,roof:0x3d2914,h:[6,12],label:'SHOP',sign:0x2ecc71,enter:true},
 brick:{cols:BRICK,roof:0x3d2914,h:[8,16],label:null,sign:null,enter:false}
};
function isClear(x,z,rad=1.2){
 for(const f of footprints) if(x+rad>f.x0&&x-rad<f.x1&&z+rad>f.z0&&z-rad<f.z1) return false;
 return true;
}
...
function init(){
 ...
 if(btn){const go=e=>{e.preventDefault();e.stopPropagation();startGame();};btn.addEventListener('click',go);btn.addEventListener('touchend',go,{passive:false});}
 ...
 scene.fog=new THREE.Fog(0xA8C8E0,60,220);
 camera=new THREE.PerspectiveCamera(68,window.innerWidth/window.innerHeight,0.15,360);
 clock=new THREE.Clock();
 scene.add(new THREE.AmbientLight(0xffffff,0.55));
 scene.add(new THREE.HemisphereLight(0xffe8c0,0x6a8a6a,0.6));
 const sun=new THREE.DirectionalLight(0xfff0d0,1.6);
 sun.position.set(60,100,40);scene.add(sun);
 const fill=new THREE.DirectionalLight(0xaaccff,0.35);
 fill.position.set(-50,40,-40);scene.add(fill);
 const sunMesh=new THREE.Mesh(new THREE.SphereGeometry(10,10,10),new THREE.MeshBasicMaterial({color:0xfff2a0,fog:false}));
 sunMesh.position.copy(sun.position).normalize().multiplyScalar(180);scene.add(sunMesh);
 try{
  buildGround();placeRoads();placeBuildings();placeSkyline();placeProps();
  spawnCars();spawnNpcs();spawnAnimals();
  buildPlayerBody();buildInteriorRoom();
 }catch(e){console.error('[City] build',e);}
 console.log('[City] BUILD v20 kids',scene.children.length,'doors',doors.length,'TL',trafficLights.length,'cars',cars.length);
...
function playFootstep(){
 if(!audioCtx)return;
 const t=audioCtx.currentTime,buf=audioCtx.createBuffer(1,audioCtx.sampleRate*0.05,audioCtx.sampleRate);
 const d=buf.getChannelData(0);for(let i=0;i<d.length;i++)d[i]=(Math.random()*2-1)*Math.exp(-i/(d.length*0.25));
 const s=audioCtx.createBufferSource();s.buffer=buf;
 const g=audioCtx.createGain();g.gain.setValueAtTime(0.07,t);g.gain.exponentialRampToValueAtTime(0.001,t+0.05);
...
function buildGround(){
 ...
 const cell=CONFIG.blockSize+CONFIG.streetWidth,sw=CONFIG.streetWidth;
 for(let i=0;i<pos.count;i++){
  const x=pos.getX(i),z=pos.getZ(i);
  const h=Math.sin(x*0.025)*Math.cos(z*0.022)*0.15;
  pos.setY(i,h);heightData[i]=h;
  const mx=((x%cell)+cell)%cell,mz=((z%cell)+cell)%cell;
  const street=mx<sw||mz<sw;
  const side=(!street)&&((mx<sw+1.4)||(mz<sw+1.4)||(mx>cell-1.4)||(mz>cell-1.4));
  if(street){cols[i*3]=0.22;cols[i*3+1]=0.23;cols[i*3+2]=0.25;}
  else if(side){cols[i*3]=0.55;cols[i*3+1]=0.55;cols[i*3+2]=0.52;}
  else{cols[i*3]=0.45;cols[i*3+1]=0.52;cols[i*3+2]=0.42;}
 }
...
function pickType(bi){
 const s=['hospital','school','police','restaurant','cafe','shop','house','brick','office'];
 if(bi<s.length)return s[bi];
 const r=Math.random();
 if(r<0.2)return 'brick';if(r<0.35)return 'shop';if(r<0.45)return 'house';
 if(r<0.55)return 'cafe';if(r<0.62)return 'restaurant';if(r<0.78)return 'office';return 'brick';
}
function placeRoads(){
 const cell=CONFIG.blockSize+CONFIG.streetWidth,half=CONFIG.worldSize*0.4,sw=CONFIG.streetWidth;
 ...
 for(let g=-half;g<=half;g+=cell){
  const zc=g+sw*0.5,xc=g+sw*0.5;
  for(let x=-half;x<half;x+=3.6){
   const dash=new THREE.Mesh(new THREE.BoxGeometry(1.8,0.06,0.14),lineM);
   dash.position.set(x+0.9,0.05,zc);scene.add(dash);
  }
  for(let z=-half;z<half;z+=3.6){
   const dash=new THREE.Mesh(new THREE.BoxGeometry(0.14,0.06,1.8),lineM);
   dash.position.set(xc,0.05,z+0.9);scene.add(dash);
  }
 }
 let tl=0;
 for(let gx=-half;gx<=half;gx+=cell){
  for(let gz=-half;gz<=half;gz+=cell){
   ...
   if(Math.abs(ix)<10&&Math.abs(iz)<10)continue;
   for(let s=-4;s<=4;s++){
    const a=new THREE.Mesh(new THREE.BoxGeometry(0.55,0.07,sw*0.9),zebraM);
    a.position.set(ix+s*0.72,0.055,iz);scene.add(a);
    const b=new THREE.Mesh(new THREE.BoxGeometry(sw*0.9,0.07,0.55),zebraM);
    b.position.set(ix,0.055,iz+s*0.72);scene.add(b);
   }
   if(tl<20){
    addTrafficLight(ix+sw*0.42,iz+sw*0.42,0);
    addTrafficLight(ix-sw*0.42,iz-sw*0.42,Math.PI);
    tl+=2;
   }
  }
 }
...
function addTrafficLight(x,z,rotY){
 ...
 g.position.set(x,getHeight(x,z),z);g.rotation.y=rotY;scene.add(g);
...
function updateTrafficLights(dt){
 ...
 for(const tl of trafficLights){
  tl.timer+=dt;const t=tl.timer%12;const ns=t<5?0:t<6.5?1:2;
  if(ns!==tl.state){tl.state=ns;setTL(tl);}
 }
}
function lightStateNear(x,z,axis,dir){
 for(const tl of trafficLights){
  if(axis==='x'){if(Math.abs(tl.z-z)>5)continue;const a=(tl.x-x)*dir;if(a>0&&a<9&&tl.state!==0)return true;}
  else{if(Math.abs(tl.x-x)>5)continue;const a=(tl.z-z)*dir;if(a>0&&a<9&&tl.state!==0)return true;}
 }
 return false;
}
function makeBldg(typeKey,bw,bh,bd){
 ...
 const winMat=new THREE.MeshLambertMaterial({color:0x1a2a38,emissive:0x1a3040,emissiveIntensity:0.15});
 const winLit=new THREE.MeshBasicMaterial({color:0xffe8a0});
 const doorMat=new THREE.MeshLambertMaterial({color:0x2a1810});
 const frameMat=new THREE.MeshLambertMaterial({color:0x555555});
 const body=new THREE.Mesh(new THREE.BoxGeometry(bw,bh,bd),bodyMat);body.position.y=bh/2;g.add(body);
 const corn=new THREE.Mesh(new THREE.BoxGeometry(bw*1.06,0.25,bd*1.06),roofMat);corn.position.y=bh+0.1;g.add(corn);
 if(typeKey==='house'){
  const rh=1.2+Math.random()*0.4;
  const roof=new THREE.Mesh(new THREE.ConeGeometry(Math.max(bw,bd)*0.72,rh,4),roofMat);
  roof.position.y=bh+rh/2+0.15;roof.rotation.y=Math.PI/4;g.add(roof);
 }
 const rows=Math.min(6,Math.max(1,Math.floor((bh-2)/2.0)));
 const cols=Math.max(2,Math.floor(bw/1.8));
 for(let r=0;r<rows;r++)for(let c=0;c<cols;c++){
  if(Math.random()<0.12)continue;
  const wy=1.5+r*2.0;if(wy+0.4>bh-0.4)continue;
  const lit=Math.random()<0.28;
  const w=new THREE.Mesh(new THREE.BoxGeometry(0.5,0.65,0.08),lit?winLit:winMat);
  w.position.set(-bw/2+1.0+c*((bw-2)/Math.max(1,cols-1)),wy,bd/2+0.05);g.add(w);
 }
 const scols=Math.max(1,Math.floor(bd/2.2));
 for(let r=0;r<rows;r++)for(let c=0;c<scols;c++){
  if(Math.random()<0.2)continue;
  const wy=1.5+r*2.0;if(wy+0.4>bh-0.4)continue;
  const w=new THREE.Mesh(new THREE.BoxGeometry(0.08,0.55,0.45),Math.random()<0.2?winLit:winMat);
  w.position.set(bw/2+0.05,wy,-bd/2+1+c*((bd-2)/Math.max(1,scols-1)));g.add(w);
 }
 const dw=Math.min(1.2,bw*0.28),dh=Math.min(2.2,bh*0.35);
 const frame=new THREE.Mesh(new THREE.BoxGeometry(dw+0.3,dh+0.25,0.12),frameMat);
 frame.position.set(0,dh/2,bd/2+0.04);g.add(frame);
 const door=new THREE.Mesh(new THREE.BoxGeometry(dw,dh,0.1),doorMat);
 door.position.set(0,dh/2,bd/2+0.08);g.add(door);
 const handle=new THREE.Mesh(new THREE.BoxGeometry(0.1,0.1,0.14),new THREE.MeshLambertMaterial({color:0xd4a84b}));
 handle.position.set(dw*0.3,dh*0.5,bd/2+0.14);g.add(handle);
 if(typeKey==='cafe'||typeKey==='restaurant'||typeKey==='shop'){
  const aw=new THREE.Mesh(new THREE.BoxGeometry(bw*0.7,0.12,1.3),new THREE.MeshLambertMaterial({color:t.sign||0x1a7a4a}));
  aw.position.set(0,2.7,bd/2+0.65);g.add(aw);
  for(const sx of [-bw*0.28,bw*0.28]){
   const post=new THREE.Mesh(new THREE.CylinderGeometry(0.04,0.04,2.5,5),frameMat);
   post.position.set(sx,1.25,bd/2+1.1);g.add(post);
  }
 }
 if(typeKey==='brick'||typeKey==='shop'){
  const fe=new THREE.Mesh(new THREE.BoxGeometry(0.08,bh*0.7,0.9),new THREE.MeshLambertMaterial({color:0x555560}));
  fe.position.set(bw/2+0.1,bh*0.4,0);g.add(fe);
  for(let i=0;i<Math.floor(bh/3);i++){
   const pl=new THREE.Mesh(new THREE.BoxGeometry(0.5,0.06,1.0),new THREE.MeshLambertMaterial({color:0x666670}));
   pl.position.set(bw/2+0.3,1.5+i*3,0);g.add(pl);
  }
 }
 if(t.label){
  const sign=new THREE.Mesh(new THREE.BoxGeometry(Math.min(bw*0.55,3.2),0.5,0.12),new THREE.MeshBasicMaterial({color:t.sign||0x3dff9a}));
  sign.position.set(0,bh-0.7,bd/2+0.1);g.add(sign);
 }
 return g;
}
function placeBuildings(){
 ...
 for(let gx=-half;gx<half;gx+=cell){
  for(let gz=-half;gz<half;gz+=cell){
   if(Math.abs(gx)<cell*0.9&&Math.abs(gz)<cell*0.9)continue;
   if(Math.random()<0.05)continue;
   const typeKey=pickType(bi++);
   const t=BTYPES[typeKey];
   const bw=CONFIG.blockSize*(0.55+Math.random()*0.35);
   const bd=CONFIG.blockSize*(0.55+Math.random()*0.35);
   ...
   if(t.enter&&bw>5){
    ...
    doors.push({type:typeKey,min:new THREE.Vector3(cx-0.9,py,cz+bd/2-0.3),max:new THREE.Vector3(cx+0.9,py+2.5,cz+bd/2+1.1),exitSpot:new THREE.Vector3(cx,py+CONFIG.playerHeight,cz+bd/2+1.6)});
    enterCount++;
   }else{
    colliders.push({min:new THREE.Vector3(cx-bw/2-0.1,py,cz-bd/2-0.1),max:new THREE.Vector3(cx+bw/2+0.1,py+bh+1,cz+bd/2+0.1)});
   }
  }
 }
 console.log('[City] doors',enterCount);
}
function placeSkyline(){
 const cols=[0x3a4a5a,0x4a5a6a,0x2a3a4a,0x5a6a7a,0xb85c38];
 for(let i=0;i<28;i++){
  const a=Math.random()*Math.PI*2;
  const d=95+Math.random()*40;
  const x=Math.cos(a)*d,z=Math.sin(a)*d;
  const w=4+Math.random()*8,h=20+Math.random()*35,dpth=4+Math.random()*8;
  const m=new THREE.Mesh(new THREE.BoxGeometry(w,h,dpth),new THREE.MeshLambertMaterial({color:cols[i%cols.length]}));
...
function placeProps(){
 ...
 const half=CONFIG.worldSize*0.36,cell=CONFIG.blockSize+CONFIG.streetWidth;
 for(let gx=-half;gx<half;gx+=cell){
  for(let gz=-half;gz<half;gz+=cell){
   ...
   const bulb=new THREE.Mesh(new THREE.SphereGeometry(0.18,5,4),glowMat);
   bulb.position.set(px,py+3.3,pz);scene.add(bulb);
  }
 }
 let trees=0;
 for(let i=0;i<80&&trees<30;i++){
  const a=Math.random()*Math.PI*2,d=12+Math.random()*(CONFIG.worldSize*0.32);
  ...
  const leaf=new THREE.Mesh(new THREE.SphereGeometry(0.9+Math.random()*0.3,7,6),new THREE.MeshLambertMaterial({color:0x2d8a3e}));
...
function makeCar(color,kind){
 ...
 const body=new THREE.Mesh(new THREE.BoxGeometry(L,H,W),bodyMat);body.position.y=0.35+H*0.15;g.add(body);
...
function spawnCars(){
 ...
 const cell=CONFIG.blockSize+CONFIG.streetWidth,half=CONFIG.worldSize*0.34;
 let id=0;
 for(let gz=-half;gz<half;gz+=cell){
  if(id>=12)break;
  ...
  body.position.set((Math.random()-0.5)*CONFIG.worldSize*0.55,0.5,gz+CONFIG.streetWidth*0.5+(dir>0?-1.4:1.4));
  body.rotation.y=dir>0?Math.PI/2:-Math.PI/2;scene.add(body);
  cars.push({mesh:body,axis:'x',dir,speed:6+Math.random()*5,baseSpeed:6+Math.random()*5,bound:half});id++;
 }
 for(let gx=-half;gx<half;gx+=cell){
  if(id>=24)break;
  const kind=id%5===0?'taxi':id%7===0?'bus':'car';
  ...
  const body=makeCar(col,kind);
  const dir=id%2===0?1:-1;
  body.position.set(gx+CONFIG.streetWidth*0.5+(dir>0?-1.4:1.4),0.5,(Math.random()-0.5)*CONFIG.worldSize*0.55);
  body.rotation.y=dir>0?0:Math.PI;scene.add(body);
  cars.push({mesh:body,axis:'z',dir,speed:6+Math.random()*5,baseSpeed:6+Math.random()*5,bound:half});id++;
 }
}
function updateCars(dt){
 ...
 for(const c of cars){
  ...
  if(lightStateNear(px,pz,c.axis,c.dir))c.speed=Math.max(0,c.speed-20*dt);
  else c.speed+=(c.baseSpeed-c.speed)*Math.min(1,dt*2.2);
  if(c.axis==='x'){c.mesh.position.x+=c.dir*c.speed*dt;if(Math.abs(c.mesh.position.x)>c.bound)c.mesh.position.x=-Math.sign(c.mesh.position.x)*c.bound;}
  else{c.mesh.position.z+=c.dir*c.speed*dt;if(Math.abs(c.mesh.position.z)>c.bound)c.mesh.position.z=-Math.sign(c.mesh.position.z)*c.bound;}
  c.mesh.position.y=getHeight(c.mesh.position.x,c.mesh.position.z)+0.02;
...
function spawnNpcs(){
 ...
 for(let i=0;i<28;i++){
  ...
  let x,z,tries=0;
  do{x=(Math.random()-0.5)*half*2;z=(Math.random()-0.5)*half*2;tries++;}while(!isClear(x,z,0.6)&&tries<20);
...
function updateNpcs(dt){
 ...
 for(const n of npcs){
  ...
  if(n.timer<=0){const a=Math.random()*Math.PI*2,s=0.8+Math.random();n.vx=Math.cos(a)*s;n.vz=Math.sin(a)*s;n.timer=1.5+Math.random()*3;}
...
function buildInteriorRoom(){
 interiorGroup=new THREE.Group();interiorGroup.position.copy(INT_ORIGIN);
 ...
 interiorGroup.visible=false;scene.add(interiorGroup);
...
function fillInterior(type){
 ...
 function box(w,h,d,mat,x,y,z){const m=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),mat);m.position.set(x,y,z);interiorFurniture.add(m);}
 if(type==='cafe'||type==='restaurant'){
  box(4,1.0,0.7,wood,0,0.5,-1.5);
  for(const [tx,tz] of [[-2.5,1.5],[0,1.5],[2.5,1.5]]){const t=new THREE.Mesh(new THREE.CylinderGeometry(0.45,0.45,0.7,8),wood);t.position.set(tx,0.35,tz);interiorFurniture.add(t);}
 }else if(type==='hospital'){box(2.5,1.05,0.7,white,0,0.52,-1.5);for(const tx of [-2.5,2.5])box(2.2,0.5,1.0,white,tx,0.55,0.5);}
 else if(type==='school'){for(let i=0;i<4;i++)box(1.2,0.7,0.55,wood,-3+i*2,0.35,1.0);}
 else if(type==='police'){box(2.5,1.0,0.7,dark,0,0.5,-1.5);}
 else if(type==='house'){box(2.0,0.5,1.5,wood,-2.5,0.4,0);box(2.0,0.15,1.5,new THREE.MeshLambertMaterial({color:0x5b8def}),-2.5,0.7,0);}
 else box(3,1.0,0.7,wood,0,0.5,-1.5);
}
...
function updateInteract(){
 if(!interactEl)return;currentDoorTarget=null;getLookDir(_lookDir);const eye=player.pos;
 if(!interiorActive){
  ...
  for(const d of doors){
   ...
   _tmp.set(cx,cy,cz).sub(eye);const dist=_tmp.length();
   if(dist>INTERACT_DIST||dist<0.001)continue;_tmp.multiplyScalar(1/dist);
   const dot=_tmp.dot(_lookDir);if(dot>bestDot){bestDot=dot;best=d;}
  }
  currentDoorTarget=best;interactEl.textContent='OPEN DOOR';
 }else if(INTERIOR_EXIT_WORLD){
  _tmp.copy(INTERIOR_EXIT_WORLD).sub(eye);const dist=_tmp.length();
  if(dist<=INTERACT_DIST&&dist>0.001){_tmp.multiplyScalar(1/dist);if(_tmp.dot(_lookDir)>INTERACT_DOT)currentDoorTarget='exit';}
  interactEl.textContent='EXIT';
 }
 interactEl.classList.toggle('hidden',!currentDoorTarget);
}
function doInteract(){
 if(!currentDoorTarget)return;playUiClick();
 if(currentDoorTarget==='exit')exitInterior();else enterInterior(currentDoorTarget);
 currentDoorTarget=null;if(interactEl)interactEl.classList.add('hidden');
}
function enterInterior(door){
 interiorExitPos=door.exitSpot.clone();interiorActive=door.type;outdoorColliders=colliders;colliders=[];
 fillInterior(door.type);interiorGroup.visible=true;
 player.pos.set(INT_ORIGIN.x,INT_ORIGIN.y+CONFIG.playerHeight,INT_ORIGIN.z+2.5);player.vel.set(0,0,0);
 scene.fog.near=30;scene.fog.far=40;scene.background=new THREE.Color(0x2a2a30);
}
function exitInterior(){
 if(!interiorActive)return;interiorGroup.visible=false;interiorActive=null;colliders=outdoorColliders||[];
 player.pos.copy(interiorExitPos);player.vel.set(0,0,0);
 scene.fog.near=60;scene.fog.far=220;scene.background=new THREE.Color(0x87B8D8);
}
function buildPlayerBody(){
 ...
 const pants=new THREE.MeshLambertMaterial({color:0x2c3e50});
 const torso=new THREE.Mesh(new THREE.BoxGeometry(0.38,0.48,0.24),shirt);torso.position.y=1.05;playerBody.add(torso);
 const head=new THREE.Mesh(new THREE.SphereGeometry(0.16,8,6),skin);head.position.y=1.45;playerBody.add(head);
 ...
 const legLM=new THREE.Mesh(new THREE.BoxGeometry(0.14,0.55,0.14),pants);legLM.position.y=-0.28;legL.add(legLM);playerBody.add(legL);
 const legR=new THREE.Group();legR.position.set(0.12,0.8,0);
 const legRM=new THREE.Mesh(new THREE.BoxGeometry(0.14,0.55,0.14),pants);legRM.position.y=-0.28;legR.add(legRM);playerBody.add(legR);
 const armL=new THREE.Group();armL.position.set(-0.26,1.2,0);
 const armLM=new THREE.Mesh(new THREE.BoxGeometry(0.11,0.45,0.11),shirt);armLM.position.y=-0.22;armL.add(armLM);playerBody.add(armL);
 const armR=new THREE.Group();armR.position.set(0.26,1.2,0);
 const armRM=new THREE.Mesh(new THREE.BoxGeometry(0.11,0.45,0.11),shirt);armRM.position.y=-0.22;armR.add(armRM);playerBody.add(armR);
 playerBody.userData={legL,legR,armL,armR,torso};playerBody.visible=false;scene.add(playerBody);
}
function updatePlayerAnim(dt,moving,speed){
 if(!playerBody)return;playerBody.visible=camMode!==0;
 ...
 const swing=Math.sin(walkPhase)*0.55*walkAmt,bob=Math.abs(Math.sin(walkPhase*2))*0.04*walkAmt;
 const ud=playerBody.userData;
 ud.legL.rotation.x=swing;ud.legR.rotation.x=-swing;ud.armL.rotation.x=-swing*0.8;ud.armR.rotation.x=swing*0.8;
 ud.torso.position.y=1.05+bob;
 playerBody.position.set(player.pos.x,player.pos.y-CONFIG.playerHeight,player.pos.z);playerBody.rotation.y=player.yaw;
}
function setupControls(){
 ...
 window.addEventListener('keydown',e=>{keys[e.code]=true;if(e.code==='Space'){e.preventDefault();tryJump();}if(e.code==='KeyC')cycleCamera();if(e.code==='KeyE'||e.code==='KeyF')doInteract();});
...
function updatePlayer(dt){
 ...
 if(!interiorActive){const bound=CONFIG.worldSize*0.46;nx=THREE.MathUtils.clamp(nx,-bound,bound);nz=THREE.MathUtils.clamp(nz,-bound,bound);}
 ...
 updatePlayerAnim(dt,moving,spd);updateInteract();updateCamera();
}
...
function animate(){
 ...
 if(started){updatePlayer(dt);updateCars(dt);updateNpcs(dt);updateAnimals(dt);updateTrafficLights(dt);}
 else{const t=clock.elapsedTime;camera.position.set(Math.sin(t*0.1)*30,16,Math.cos(t*0.1)*30);camera.lookAt(0,4,0);}
